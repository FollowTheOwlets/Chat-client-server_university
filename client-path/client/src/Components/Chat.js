    ...
    const sendMessage =(event) =>{
        ...
        if(text.toString().trim().length!==0){
            ...
            for (let char of textMessage) {
                newText += char;
                ((i > 25 && char === " ") ||
                (i > 30 && textMessage[i+1] !== " " && textMessage[i+2] !== " " && textMessage[i+3] !== " " && textMessage[i+4] !== " ")) &&
                (newText += "\n" ) && (i=0);
                i++;
            }
            socket.emit('send-message', {senderName,"text":newText})
            addMessageToRoom({senderName,"text":newText})
        }else{
